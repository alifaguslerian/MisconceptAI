...
const STATUS_CONFIG = {
  correct:   { label: "Benar",   bg: "bg-correct/15",   text: "text-correct",   border: "border-correct/25",   dot: "bg-correct" },
  partial:   { label: "Parsial", bg: "bg-partial/15",   text: "text-partial",   border: "border-partial/25",   dot: "bg-partial" },
  incorrect: { label: "Keliru",  bg: "bg-incorrect/15", text: "text-incorrect", border: "border-incorrect/25", dot: "bg-incorrect" },
  missing:   { label: "Kosong", bg: "bg-surface2",      text: "text-missing",   border: "border-border",       dot: "bg-missing" },
}
...
function HeatmapCell({ concept, status, evidence, suggestion }) {
  ...
  return (
    <div className="relative" onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}>
      <div className={`w-full h-12 rounded border ${cfg.bg} ${cfg.border} flex items-center justify-center cursor-default transition-all`}>
      ...
      {hovered && (evidence || suggestion) && (
        <div className="absolute z-50 bottom-full left-1/2 -translate-x-1/2 mb-2 w-64 bg-surface2 border border-border rounded p-3 shadow-xl pointer-events-none">
          <p className={`text-xs font-mono font-medium ${cfg.text} mb-1`}>{concept}</p>
          {evidence && (
...
function MisconceptionBadge({ type }) {
  if (!type) return null
  const map = {
    factual_error:         { label: "Faktual Keliru",    cls: "bg-incorrect/10 text-incorrect border-incorrect/20" },
    partial_understanding: { label: "Parsial",           cls: "bg-partial/10 text-partial border-partial/20" },
    overgeneralization:    { label: "Overgeneralisasi",  cls: "bg-partial/10 text-partial border-partial/20" },
    conceptual_confusion:  { label: "Konfusi Konsep",    cls: "bg-incorrect/10 text-incorrect border-incorrect/20" },
    missing_concept:       { label: "Konsep Hilang",     cls: "bg-surface2 text-missing border-border" },
    underestimation:       { label: "Meremehkan",        cls: "bg-partial/10 text-partial border-partial/20" },
  }
  const m = map[type] ?? { label: type, cls: "bg-surface2 text-missing border-border" }
  return (
...
export default function SessionResults() {
  ...
  useEffect(() => {
    axios.get(`${API}/sessions/${id}/analyze/`)
      .then(r => {
        setData(r.data)
  ...
  if (!data || data.results.length === 0) return (
    <div className="max-w-2xl mx-auto text-center py-20">
      <p className="text-gray-500 font-mono text-sm mb-4">Belum ada hasil analisis untuk sesi ini.</p>
      <button
  ...
  const matrix = data.results.map(r => {
    ...
    r.analysis.concepts.forEach(c => {
      conceptMap[c.concept_name.trim()] = c
    })
  ...
  return (
    <div className="max-w-5xl mx-auto">
      ...
      <div className="mb-8 flex items-start justify-between">
        <div>
          <p className="text-xs font-mono text-missing uppercase tracking-widest mb-1">Hasil Analisis · Sesi #{id}</p>
          <h1 className="font-sora font-bold text-2xl text-white">{data.session_name}</h1>
          <p className="text-sm text-gray-500 mt-1">{data.results.length} mahasiswa · {concepts.length} konsep</p>
        </div>
        <button
          onClick={() => navigate(`/sessions/${id}/answers`)}
      ...
      <div className="grid grid-cols-4 gap-3 mb-8">
        {[
          ...
          <div key={s.label} className="bg-surface border border-border rounded p-4">
            ...
            <p className="text-xs text-gray-500 font-mono mt-0.5">{s.label} · {total > 0 ? Math.round(s.value/total*100) : 0}%</p>
          </div>
        ))}
      </div>
      ...
      <div className="bg-surface border border-border rounded p-5 mb-8 overflow-x-auto">
          ...
          <thead>
            <tr>
              <th className="text-left pr-4 pb-3 w-36">
                <span className="text-xs font-mono text-gray-600">Mahasiswa</span>
              </th>
              {concepts.map(c => (
                <th key={c} className="pb-3 px-1 min-w-28">
                  <span className="text-xs font-mono text-gray-400 block text-center leading-tight">{c}</span>
                </th>
              ))}
            </tr>
          </thead>
            ...
            {matrix.map(({ student, conceptMap }) => (
              <tr key={student}>
                <td className="pr-4 py-1">
                  <button
                    onClick={() => setSelected(data.results.find(r => r.student_name === student))}
                    className={`text-sm text-left font-medium transition-colors hover:text-accent ${
                      selected?.student_name === student ? "text-accent" : "text-gray-300"
                    }`}
                  >
                    {student}
                  </button>
        ...
        <p className="text-xs text-gray-600 font-mono mt-3">hover cell untuk lihat evidence & saran remedial</p>
      </div>
      ...
      {selected && (
        <div className="bg-surface border border-border rounded p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-xs font-mono text-gray-600">{selected.analyzed_at?.slice(0, 10)}</span>
          </div>
          ...
          <div className="bg-surface2 border border-border rounded px-4 py-3 mb-5">
            ...
            <p className="text-sm text-gray-300 leading-relaxed">{selected.analysis.summary}</p>
          </div>
          ...
          <div className="space-y-3">
            {selected.analysis.concepts.map(c => {
              ...
              return (
                <div key={c.concept_name} className={`border ${cfg.border} rounded p-4`}>
                  <div className="flex items-center gap-2 mb-2">
                    <span className={`w-2 h-2 rounded-full ${cfg.dot}`} />
                    <span className="text-sm font-mono font-medium text-white">{c.concept_name}</span>
                    <span className={`text-xs font-mono ${cfg.text}`}>{cfg.label}</span>
                    <MisconceptionBadge type={c.misconception_type} />
                  </div>
                  {c.evidence && (
                    <p className="text-xs text-gray-500 italic mb-1.5">"{c.evidence}"</p>
                  )}
                  {c.suggestion && (
                    <p className="text-xs text-gray-400 leading-relaxed">{c.suggestion}</p>
                  )}
                </div>
              )
            })}
